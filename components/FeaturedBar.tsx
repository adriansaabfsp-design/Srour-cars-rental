    ...
    <div className="mt-16 w-full max-w-5xl mx-auto">
      ...
      <div className="mb-5 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <svg className="h-4 w-4 text-terra" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
          </svg>
          <span className="text-[11px] font-bold uppercase tracking-[0.3em] text-terra/80">
            Featured
          </span>
        </div>
      ...
      >
        {visibleCars.map((car) => (
          <Link
            key={car.id}
            href={`/cars/${car.id}`}
            className="group relative overflow-hidden border border-white/[0.06] bg-white/[0.03] backdrop-blur-sm transition-all duration-500 hover:border-terra/30 hover:bg-white/[0.06]"
          >
            {/* Image */}
            <div className="relative aspect-[16/9] w-full overflow-hidden bg-black">
              ...
              {/* Price badge */}
              <div className="absolute top-0 right-0 bg-terra px-3 py-1.5">
                <span className="text-sm font-extrabold text-white">${car.price}</span>
                <span className="text-[9px] font-bold text-white/50">/day</span>
              </div>

              {/* Bottom info */}
              <div className="absolute bottom-0 left-0 right-0 p-4">
                <div className="mb-1 text-[9px] font-bold uppercase tracking-[0.3em] text-terra/80">
                  {car.brand}
                </div>
            ...
            <div className="absolute top-1/2 right-3 -translate-y-1/2 opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0 translate-x-2">
              <div className="flex h-8 w-8 items-center justify-center border border-terra/30 bg-black/60 backdrop-blur-sm">
                <svg className="h-3.5 w-3.5 text-terra" fill="none" stroke="currentColor" viewBox="0 0 24 24">
